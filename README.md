...
바이브 코더는 Claude Code CLI를 Slack에서 자연어로 직접 사용할 수 있게 만든 개발 보조 봇이에요. 코드 분석, 버그 수정, 리팩터링, 구조 설명은 물론 패치 생성까지 멘션 만으로 처리해줘서 IDE 없이도 빠르고 유연하게 협업할 수 있고, 팀 누구나 편하게 코드 이해와 수정을 요청할 수 있는 가벼운 대화형 엔지니어처럼 동작해요.

## 사용법

1. 바이브 코더는 멘션에만 반응합니다. 채널에 초대한 뒤 @바이브 코더를 멘션해서 프롬프트를 전달해주세요.
2. 바이브 코더의 응답 메시지에 회신(스레드)을 달면 Claude Code 세션을 유지하면서 대화를 이어나갈 수 있어요.
3. 바이브 코더와 무관하게 시작된 스레드에서도 바이브 코더를 호출하면 그 스레드 내에서 세션이 유지됩니다.
